...
from spiral import PolarNet, RawNet, WrappedCart2Polar
from spiral import graph_all_nodes, graph_hidden
...
parser.add_argument('--net',type=str,default='raw',help='polar, polar_mod or raw')
...
if args.net == 'polar':
    ...
    print(f'error: unknown model type {args.net}')
    exit(1)
...
if not list(net.parameters()):
    print(f"error: net {args.net} has no trainable params")
    exit(1)
...
if not args.infer:
    try:
        for epoch in range(1, args.epochs):
            accuracy = train(net, train_loader, optimizer, epoch)
...
os.makedirs(net_prefix, exist_ok=True)

# graph hidden units
for name in graph_all_nodes(net):
    plt.scatter(full_input[:,0],full_input[:,1],
                c=1-full_target[:,0],cmap='RdYlBu')
    plt.title(f"node_{name}")
    plt.savefig(f'{net_prefix}/node_{name}.png')
