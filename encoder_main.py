...
from encoder_image import encode_image
...
parser.add_argument('--no-load',action="store_true",default=False,help='reload model')
...
if args.target == 'input':
    ...
    import cv2
    im = cv2.imread(args.target, cv2.IMREAD_GRAYSCALE)
    if im is None:
        print(f"error: unable to load target image '{args.target}'")
        exit()
    _, im = cv2.threshold(im, 127, 255, cv2.THRESH_BINARY_INV)
    target = torch.Tensor(encode_image(im))
    fname = os.path.basename(args.target)
    fname = os.path.splitext(fname)[0]
    target_name = f"image_{fname}"
else:
...
net_fname = f"encoder_{target_name}.pt"
plot_fname = f"encoder_{target_name}.png"

if os.path.exists(net_fname) and not args.no_load:
    print(f"info: loading model {net_fname}")
    net.load_state_dict(torch.load(net_fname))
else:
    print(f"info: creating model {net_fname}")
...
PLOT_EPOCHS = set([50,100,150,200,300,500,700,1000,
...
if args.infer:
    plot_hidden(net)
    plt.title(f"hidden unit diagram for {target_name}")
    plt.xlabel("x")
    plt.ylabel("y")
...
try:
    ...
    if args.save:
        print(f'saving model {net_fname}')
